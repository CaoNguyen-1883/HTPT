            ...
            onConnect: () => {
                ...
                client.subscribe("/topic/migrations", (message) => {
                    ...
                    if (migration.status === "PENDING") {
                        ...
                        addLog(
                            `Migration started: ${migration.sourceNodeId} -> ${migration.targetNodeId}`,
                            "info",
                        );
                    } else {
                        ...
                        if (migration.status === "COMPLETED") {
                            addLog(
                                `Migration completed: ${migration.id}`,
                                "success",
                            );
                        } else if (migration.status === "FAILED") {
                            addLog(
                                `Migration failed: ${migration.errorMessage}`,
                                "error",
                            );
                        }
                    }
